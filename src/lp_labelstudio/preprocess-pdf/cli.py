...
def extract_images(input_pdf: Path, output_dir: Path, force: bool=False):
    ...
    for page_num, page in enumerate(doc, start=1):
        ...
        if len(image_list) != 1:
            raise ValueError(f'Page {page_num:02d} has {len(image_list)} images. Expected 1 image per page.')
        ...
        for img_index, img in enumerate(image_list, start=1):
            ...
            if not force and output_path.exists():
                logging.info(f'Skipping page {page_num:02d}, file already exists: {output_path}')
                continue
...
def cli_extract_images(input_pdf: Path, output_dir: Path, log_level: str, force: bool=False):
    """
