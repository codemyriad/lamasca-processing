...
def generate_thumbnails(source_folder: str, destination_folder: str):
    ...
    with Pool(processes=concurrency) as pool:
        with click.progressbar(
            length=total_tasks, label="Generating thumbnails"
        ) as progress_bar:
...
def process_image(args):
    ...
    def draw_debug_info(draw, zone, x, y, debug_info):
        ...
        draw.rectangle(
            [x, y - text_height - 5, x + text_width, y - 5], fill=(255, 255, 255, 200)
        )

        # Draw each line of debug text
        for i, line in enumerate(debug_text):
            draw.text(
                (x, y - text_height + (i * 15)), line, fill=(0, 0, 0, 255), font=font
            )
        ...
        if boxes:
            ...
            for article_idx, article in enumerate(articles):
                ...
                for zone in article:
                    ...
                    draw.rectangle(
                        [x, y, x + width, y + height], fill=color + (OPACITY,)
                    )
